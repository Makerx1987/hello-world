...
uint16_t	POINT_COLOR = WHITE;	//画笔颜色	默认为白色
uint16_t	BACK_COLOR 	= BLACK;	//背景颜色	默认为黑色

void lcdinitdelay(int count)
{
	unsigned int i;
	for(i=0;i<count;i++);
}
...
{
	return HAL_SPI_Transmit(&SPI1_Handler,TxData,size,10);
}
...
static void LCD_Write_Cmd(uint8_t cmd)
{
	...
	HAL_SPI_Transmit(&SPI1_Handler,cmdbuffer,1,HAL_MAX_DELAY);
}
...
static void LCD_Write_Data(uint8_t data)
{
	...
	HAL_SPI_Transmit(&SPI1_Handler,databuffer,1,HAL_MAX_DELAY);
}
...
void LCD_Clear(uint16_t color)
{
    ...
    {
        SPI1_WriteData(lcd_buf, LCD_Buf_Size);
    }
}
...
void LCD_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    ...
    if(y1 == y2)
    {
        ...
        LCD_WR(1);
        SPI1_WriteData(lcd_buf, (x2 - x1) * 2);
        return;
    }
...
void LCD_ShowChar(uint16_t x, uint16_t y, char chr, uint8_t size)
{
    ...
    if((size == 16) || (size == 24)|| (size == 32) )	//16和32号字体
    {
...
void LCD_Show_Image(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const uint8_t *p)
{
    ...
    //SPI_WriteData((uint8_t *)p, width * height * 2);
	HAL_SPI_Transmit(&SPI1_Handler,(uint8_t *)p,width * height * 2,1000);
}
...
void ali_LCD_Address_Set(uint16_t x1, uint16_t y1)
{
    ...
    LCD_Write_Data(x1);
    LCD_Write_Data(0);
    LCD_Write_Data(x1+0xef);

    LCD_Write_Cmd(0x2b);
    LCD_Write_Data(0);
    LCD_Write_Data(y1);
    LCD_Write_Data(0);
    LCD_Write_Data(y1+0xef);
...
void BSP_LCD_Clear(uint16_t Color)
{
	...
	{
		ali_LCD_Address_Set(0,counter);
		
		LCD_WR(1);
		HAL_SPI_Transmit(&SPI1_Handler, black_gui, 480, 10); 
	}
}
