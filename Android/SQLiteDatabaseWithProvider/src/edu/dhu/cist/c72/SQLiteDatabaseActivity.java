...
public class SQLiteDatabaseActivity extends Activity implements OnClickListener {
	...
	public void onClick(View v) {
		...
		myData helper = new myData(SQLiteDatabaseActivity.this, this.getFilesDir() + "/test.db");
		...
		case R.id.id_databaseButtonQuery:// 查询所有元素依次弹窗
			// String zsname = "zs";
			// Cursor cursor = database.query("MYSTUDENTDB", null,
			// "CLASS= ? AND NAME=?", new String[]{"1","zs"}, null, null,
			// null);//NAME='zs'
			// Cursor cursor = database.query("MYSTUDENTDB", null, "NAME=?", new
			// String[]{"zs"}, null, null, null);
			...
			if (cursor.getCount() > 0) {
				while (cursor.moveToNext()) {
					...
					sb.append(cursor.getString(0).toString() + "_" + id + name + age + address + "\r\n");
				}
